...
Type maptype
    ...
    y As Single
    shape As Long
    halfx As Long 'halfx=1 时，地图缺左边，=2时，地图缺右边
    halfy As Long 'halfy=1 时，地图缺上边，=2 时，地图缺下边
End Type
Type Boxtype
    x As Single
    y As Single
...
Dim ccfang(3 To 10) As Long '随机数，以决定是否改变方向
Dim ccshoot(3 To 10) As Long '随机数，以决定是否射击
...
If i >= 3 And notmove(i) <> 0 Then
    ccfang(i) = ccfang(i) - 2
End If
If i <= 2 And box.shape <> 0 Then '捡盒子
